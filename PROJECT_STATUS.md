...
26. **Haptics** (requested 2026-06-21):
    ...
    - **What we did:** the **FAB (+)** is now a `<div class="fab">` containing an invisible full-size native `#fab-switch` (`opacity:0`, NOT `appearance:none` or the haptic dies). Tapping + physically toggles it → real iOS tick; its `onchange` calls `openAddTxSheet()`. **Needs on-device confirmation** (can't test haptics in the desktop preview). To extend to other controls, overlay the same invisible switch on each tappable element. The honest "rich haptics everywhere" path is a native wrapper (Capacitor) exposing `UIImpactFeedbackGenerator`.
