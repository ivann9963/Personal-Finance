...
26. **Haptics helper** (requested 2026-06-21): `haptic(kind)` in `ui-components.js` with named presets (`light/medium/heavy/success/warning/error`) over `navigator.vibrate`, guarded so it's a safe no-op where unsupported. Wired through the action chokepoints: **`showToast`** (success/warning/error/info patterns — covers most "something happened" feedback), **`openSheet`** (light tap on any sheet open), **`confirmDialog`** OK (medium), and the **drag pickups** (category + analytics-section reorder → medium "lift"). ⚠️ **No-op on iOS Safari/PWA** (no Vibration API) — fires on Android Chrome only.
